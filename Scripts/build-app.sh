...
APP_DIR="$ROOT_DIR/.build/Watch.app"
...
cp "$ROOT_DIR/.build/debug/WatchApp" "$MACOS_DIR/Watch"
chmod +x "$MACOS_DIR/Watch"
...
iconutil -c icns "$ICONSET_DIR" -o "$RESOURCES_DIR/WatchIcon.icns"

RESOURCE_BUNDLE="$ROOT_DIR/.build/debug/Watch_WatchApp.bundle"
if [[ -d "$RESOURCE_BUNDLE" ]]; then
  cp -R "$RESOURCE_BUNDLE" "$APP_DIR/Watch_WatchApp.bundle"
fi
...
<dict>
  ...
  <string>app.watch.prototype</string>
